...
// claimSolver uses an op-plasma provider to solve a da-challenge move.
type claimSolver struct {
	client plasma.DAClient
}

// newClaimSolver creates a new claimSolver using the provided plasma.DAClient.
func newClaimSolver(client plasma.DAClient) *claimSolver {
	return &claimSolver{
		client: client,
	}
}
